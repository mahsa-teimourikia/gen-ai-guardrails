...
decision = policy.evaluate(request, evidence)`,
    checkpoints: [
      ...
      },
      {
        ...
        options: ["The vendor's marketing claims", "Allowed inputs", "A promise that the model is safe"],
        answer: 1,
      },
      {
        question: "Which evidence makes a decision auditable?",
        options: ["Only the final answer", "An unversioned prompt", "Reason codes and a trace ID"],
        answer: 2,
      },
    ],
  },
  {
    ...
    checkpoints: [
      ...
      },
      {
        ...
        options: ["It makes every metric perfect", "Rates need their population defined", "It replaces regression cases"],
        answer: 1,
      },
      {
        question: "What is policy v2 evidence?",
        options: ["A hidden prompt edit", "A screenshot without inputs", "A versioned decision record"],
        answer: 2,
      },
    ],
  },
...
if not release_gate.all_required_checks_pass():
    ...
    checkpoints: [
      {
        question: "What stops repeated calls to a degraded dependency?",
        options: ["A circuit breaker", "A higher budget", "Unbounded retries"],
        answer: 0,
      },
      {
        question: "Which change belongs in a safety-critical change review?",
        options: ["A typo in a non-policy comment", "A safety-critical threshold change", "A color used in documentation"],
        answer: 1,
      },
      {
        question: "What makes a release gate useful?",
        options: ["One average benchmark", "An undocumented exception", "Evidence from every required evaluation layer"],
        answer: 2,
      },
    ],
  },
...
return executor.run(action, idempotency_key=action.key)`,
    checkpoints: [
      ...
      },
      {
        ...
        options: ["A longer system prompt", "Idempotency", "Ignoring the receipt"],
        answer: 1,
      },
      {
        question: "What can a moderation threshold decide?",
        options: ["Grant a user a new role", "Replace authorization", "Allow, transform, escalate, or block"],
        answer: 2,
      },
    ],
  },
...
release = bypasses == 0 and unauthorized_actions == 0`,
    checkpoints: [
      ...
      },
      {
        ...
        options: ["By its confidence alone", "Against blind human-labeled examples", "By deleting disagreements"],
        answer: 1,
      },
      {
        question: "What does the expected-loss threshold sweep combine?",
        options: ["Only raw accuracy", "The judge's confidence alone", "Weighted false-negative and false-positive costs"],
        answer: 2,
      },
    ],
  },
...
assert executed.reason_codes == ["session_revoked"]`,
    checkpoints: [
      ...
      },
      {
        ...
        options: ["Raw token values", "Masked text", "An unverified secret fragment"],
        answer: 1,
      },
      {
        question: "What detects an altered audit event?",
        options: ["A higher detector score", "A longer prompt", "Hash-chain verification"],
        answer: 2,
      },
    ],
  },
...
evidence = audit_side_effects(results, state, capabilities)`,
    checkpoints: [
      {
        question: "What two conditions gate a provision tool's phase exposure?",
      ...
      },
      {
        ...
        options: ["Ignoring the previous result", "A receipt keyed by fingerprint", "Consuming budget twice"],
        answer: 1,
      },
      {
        question: "What should happen after verification mismatch?",
        options: ["Pretend the write succeeded", "Add a new capability", "Escalate for reconciliation"],
        answer: 2,
      },
    ],
  },
